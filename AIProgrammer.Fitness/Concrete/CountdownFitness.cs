...
namespace AIProgrammer.Fitness.Concrete
{
    ...
    public class CountdownFitness : FitnessBase
    {
        ...
        {
            byte[][] targetValues = { new byte[] { 5, 4, 3, 2, 1, 0 },
                                      new byte[] { 3, 2, 1, 0 },
                                      new byte[] { 2, 1, 0 },
            ...
            for (int i = 0; i < targetValues.Length; i++)
            {
                ...
                for (int j = 0; j < targetValues[i].Length; j++)
                {
                    ...
                    {
                        byte value;
                        if (Byte.TryParse(_console[j].ToString(), out value))
                        {
                            Fitness += 256 - Math.Abs(value - targetValues[i][j]);
                        }
                    }
                }
